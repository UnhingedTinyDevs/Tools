...
These are the variables that will determine how the [PoissonDisc](#disc) class will generate the points. You can use the [PointSampler](#sampler) to view the changes live in the editor.

### DEFAULT_POINT_DEBUG_UID ([String](https://docs.godotengine.org/en/stable/search.html?q=String&check_keywords=yes&area=default)) {#default-str}
...
The scene you want to use as a debug visual . By default it is set to [DEFAULT_POINT_DEBUG_UID (String)](#default-str).
...
## PoissonDisc {#disc}
This is the class in charge of running the algorithm it is given a [PoissonSettings](#settings) resource which determines how the sampling is done.
...
and this is basically all the [PointSampler](#sampler) does is generate points on startup and when settings change and draw them to the screen. 


## PointSampler {#sampler}
The point sampler is an [Area2D]https://docs.godotengine.org/en/stable/classes/class_area2d.html() that when given [PoissonSettings](#settings) will generate points using [PoissonDisc](#disc) class. This allows you to visually define different distributions and save there settings so that you can swap them out easily.

### How To
1. add the  **Sampler Node** to the scene tree
2. give the **Sampler Node** a [PoissonSettings](#settings).
